...
*WARNING:* never do this to a commit that you've already pushed to a remote
repository.
...
into your index*. It's like going back in time to the moment right before you
committed. This is a useful technique in resuming work on an unfinished
changeset.
...
This creates a patch that is the opposite of changeset ~[hash]~ and commits it to
your repo.

If you want to revert that changeset without committing it (putting the
reversion into your index), use:
...
branch (e.g., ~origin/foo~ if the checked out branch is ~foo~).
...
after a ~git fetch~, to fetch down *all the data* it has for the *current branch* that
you do not --, the next time you do a pull.
...
#+begin_src conf
	...
	rc = "!git log ..origin/$(git name-rev --name-only HEAD)"
#+end_src
