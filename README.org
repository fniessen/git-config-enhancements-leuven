...
- ~core.autocrlf = true~
  ...
  replaced with LF when writing that file to the object database and *turn all LF
  back into CRLF when writing out into the working tree*. This is the recommended
  setting on Windows because it ensures that your repository can be used on
  other platforms while retaining CRLF in your working tree.
...
- ~core.autocrlf = input~
  ...
  and write them into the working tree they will still have LFs to denote the
  end of line. This setting is generally used on Unix/Linux/OS X to prevent
...
#+begin_src conf
    changelog1 = log --pretty=format:'%Cgreen%d %Creset%s' --date=iso
