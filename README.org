...
#+begin_src conf
    ...
    # across all branches.
    # Example: git search-commit-message-all-branches "pattern1" "pattern2" "pattern3"
    # See also git search-for-commits-or-bitbucket PATTERN... (for current branch!!!)
    search-commit-message-all-branches = "!f() { \
        if [ $# -eq 0 ]; then \
...
#+begin_src conf
    # Usage: git search-for-commits-or-bitbucket PATTERN...
    #
    ...
    search-for-commits-or-bitbucket = "!f() { \
        if [ \"$#\" -eq 0 ]; then \
            printf >&2 'Usage: git search-for-commits-or-bitbucket PATTERN...\n'; \
            return 2; \
        fi; \
        for PATTERN in \"$@\"; do \
...
#+begin_src conf
    ...
    search-for-commits-and-gitlab = "!f() { \
        if [ \"$#\" -eq 0 ]; then \
            printf >&2 'Usage: git search-for-commits-and-gitlab PATTERN...\n'; \
            return 2; \
        fi; \
...
#+begin_src conf
    # Usage: git search-commit-content-all-branches SEARCH_TERM
    # Search through the content of all commits for the exact search term.
    search-commit-content-all-branches = "!f() { \
        git log --all -S \"$1\"; \
    }; f"
#+end_src
