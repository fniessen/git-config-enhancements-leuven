...
in Git-handled repositories.  There's an option to auto-convert:

#+begin_src conf
    # Commit as LF, but does not check out as CRLF (for Shell scripts!)
    autocrlf = input
#+end_src
...
- ~core.autocrlf = false~
  ...
  immediately. The result of using false is that Git doesn’t ever mess with line
  endings on your file. You can check in files with LF or CRLF or CR or some
  random mix of those three and Git does not care. This can make diffs harder to
  read and merges more difficult. Most people working in a Unix/Linux world use
  this value because they don’t have CRLF problems and they don’t need Git to be
  doing extra work whenever files are written to the object database or written
  out into the working directory.

- ~core.autocrlf = true~

  This means that Git will process all text files and make sure that CRLF is
  replaced with LF when writing that file to the object database and *turn all
  LF back into CRLF when writing out into the working directory*. This is the
  recommended setting on Windows because it ensures that your repository can be
  used on other platforms while retaining CRLF in your working directory.

- ~core.autocrlf = input~

  This means that Git will process all text files and make sure that CRLF is
  replaced with LF when writing that file to the object database. It will *not*,
  however, *do the reverse*. When you read files back out of the object database
  and write them into the working directory they will still have LFs to denote
  the end of line. This setting is generally used on Unix/Linux/OS X to prevent
  CRLFs from getting written into the repository. The idea being that if you
  pasted code from a web browser and accidentally got CRLFs into one of your
  files, Git would make sure they were replaced with LFs when you wrote to the
  object database.
...
#+begin_src conf
    # Proxy.
    setproxy = !git config --global http.proxy http://proxy-guest.example.com:8080 \
                    && git config --global https.proxy https://proxy-guest.example.com:8080
    rmvproxy = !git config --global --unset http.proxy \
...
#+begin_src conf :tangle no
    create-repository-gh = "!f() { REPO_OWNER=$1; REPO_SLUG=$2; \
        curl -u \"$1\" \
...
#+begin_src conf
    ...
    repo-owner = "!f() { git remote show $(git origin) -n \
                             | grep 'Fetch URL:' \
                             | sed -E 's#^.*[/:](.*)/.*$#\\1#' \
                             | sed 's#.git$##'; }; f"
#+end_src
...
#+begin_src conf
    repo-slug = "!f() { git remote show $(git origin) -n \
                            | grep 'Fetch URL:' \
                            | sed -E 's#^.*/(.*)$#\\1#' \
                            | sed 's#.git$##'; }; f"
#+end_src
...
contents*) from the working directory (and its subdirectories).
...
#+begin_src conf
    showtool = "!f() { REV=$(git rev-parse \"${*:-HEAD}\"); \
                       git difftool $REV~1 $REV; }; f"
#+end_src
...
#+begin_src conf
    ...
    where2 = "!f() { git for-each-ref --format='%(committerdate:iso) %09 %(authorname) %09 %(refname:short)' --contains $1 \
                         | sort -r; }; f"
#+end_src
...
#+begin_src conf
    ...
    tags-that-contain = "!f() { git tag -l --contains $1 | sort -r; }; f"
#+end_src
...
#+begin_src conf
    browse-commits = "!f() { URL=$(git config --get remote.$(git origin).url); \
                             cygstart ${URL/.git}/commits; }; f"
#+end_src
...
#+begin_src conf
    br-active = "!f() { git branch --no-merged | grep -v '\\*'; }; f"
    branches-active = "!f() { git branch -a -vv --no-merged | grep -v '\\*'; }; f"
#+end_src
...
#+begin_src conf
    br-merged = "!f() { git branch --merged | grep -v '\\*'; }; f"
    branches-merged = "!f() { git branch -a -vv --merged | grep -v '\\*'; }; f"
#+end_src
...
#+begin_src conf
    ...
    delete-local-merged-branches = "!f() { git branch --no-color --merged | grep -v '\\*' \
...
#+begin_src conf
    delete-branch = "!f() { git delete-local-branch $1 \
...
- If we are on the branch we want to rename, we can just
  ~git rename-local-branch NEWBRANCH~.

- If we are on another branch, we have to use
  ~git rename-local-branch OLDBRANCH NEWBRANCH~.
...
#+begin_src conf
    # Usage: git rename-branch BRANCH_OLD BRANCH_NEW
...
#+begin_src conf :tangle no
    ...
    delete-all-tags = "!f() { for TAG in $(git tag); do
                                  git tag -d $TAG
                                  git push $(git origin) :$TAG
                              done;
                            }; f"
#+end_src
...
: git diff --stat $(git origin)/master...my_forked_branch
...
#+begin_src conf
    show-branch-history = "!f() { git log HEAD --not master; }; f"
...
*** TODO Diff all changes since branching from master

Show all commits in a git branch since original branching point from =master=:

#+begin_src conf
    # log since master.
    missing-master = !git log master...

    # diff since master.
    missing-master-diff = !git diff master...
#+end_src
...
#+begin_src conf
    graph = log --graph -25 --branches --remotes --tags --pretty='format:%C(yellow)%h %C(reset) %<(75,trunc)%s <%cn> (%cr) %C(red)%d' --date-order
...
#+begin_src conf
    pull-request-bb = "!f() { USERNAME=$1; \
        curl --user $USERNAME \
            https://bitbucket.org/$(git repo-owner)/$(git repo-slug)/pull-requests/new?source=$(git current-branch)&t=1; }; f"
#+end_src
...
#+begin_src conf
    # For GitHub.
    pr-checkout-gh = "!f() { git fetch $(git origin) pull/$1/head:pr/$1 \
                                 && git checkout pr/$1; }; f"
    # https://help.github.com/en/github/collaborating-with-issues-and-pull-requests/checking-out-pull-requests-locally

    # For GitLab.
    pr-checkout-gl = "!f() { git fetch $(git origin) merge-requests/$1/head:pr/$1 \
                                 && git checkout pr/$1; }; f"
...
#+begin_src conf :tangle no
    issues = "!f() { USERNAME=$1; \
        curl --user $USERNAME \
            'https://api.bitbucket.org/2.0/repositories/$(git repo-owner)/$(git repo-slug)/issues?pagelen=100' \
            ...
            | sed \"s#\"closed\"#$(tput setaf 1)&$(tput sgr0)#\"; }; f"
#+end_src
...
curl -u $USERNAME -X GET 'https://api.bitbucket.org/2.0/repositories/$(git repo-owner)/$(git repo-slug)/issues?pagelen=100' \
    | jq --compact-output '.values | .[] | [.id, .title, .type, .priority, .state, .votes, .assignee.display_name, .created_on, .updated_on]'
# curl -s -S
#+end_src
...
#+begin_src conf
    browse-issues = "!f() { URL=$(git config --get remote.$(git origin).url); \
                            cygstart ${URL/.git}/issues/$1; }; f"
#+end_src

* Wiki

#+begin_src conf
    browse-wiki = "!f() { URL=$(git config --get remote.$(git origin).url); \
                          cygstart ${URL/.git}/wiki; }; f"
#+end_src
...
#+begin_src conf
    ...
    print-reponame-then-run = "!f() { echo "======"; \
...
#TAGS
    ...
    publishtag = "!sh -c 'git push ${2:-$(git origin)} $1' -"
    # Removes given tag from remote 'origin' repo (or the remote passed as the second parameter)
    unpublishtag = "!sh -c 'git push ${2:-$(git origin)} :refs/tags/$1' -"
...
#+begin_src conf
    today = log --since=midnight --author=\"$(git config user.name)\" --no-merges --all --pretty='format:%C(green)%ad%C(auto) - %s' --date='format:%Y-%m-%d %a %H:%M'
    yesterday = log --since='1 day ago midnight' --until=midnight --author=\"$(git config user.name)\" --no-merges --all --pretty='format:%C(green)%ad%C(auto) - %s' --date='format:%Y-%m-%d %a %H:%M'
    this-week = log --since=1.weeks --author=\"$(git config user.name)\" --no-merges --all --pretty='format:%C(green)%ad%C(auto) - %s' --date='format:%Y-%m-%d %a %H:%M'
#+end_src
...
#+begin_src conf
    ...
    standup = "!git log --branches \
                        ...
                        --pretty='format:%C(green) %ad %C(yellow)%h %Creset %s %C(green bold)%d' \
                        --date=local"
...
#+begin_src conf
    related = !sh -c 'git log --follow --pretty=\"format:%H\" $1 | xargs -L 1 git diff-tree --no-commit-id --name-only -r | sort | uniq -c | sort -b -n -r'
#+end_src
...
&& git remote add $(git origin) https://github.com/"$USERNAME"/"$REPOSITORY".git \
...
#+begin_src conf
missing = "!f() { git rev-list ..$1 --oneline | sed -e '1 iOnly in other:'; \
                  git rev-list $1.. --oneline | sed -e '1 iOnly in HEAD:'; }; f"
#+end_src
...
with ~git stash branch NEW_BRANCHNAME~.
...
#+begin_src conf
    ...
    cpr = "!f() { git fsck | grep commit | cut -d' ' -f3 | \
                  ...
                  xargs -L 1 git log --format=%at:%h -1 | sort | cut -f2 -d: | xargs -L 1 git log --graph --oneline --format='%Cblue%h %Cgreen(%ar) %Creset%s' -3; }; f"
#+end_src
...
#
# #+begin_src shell :tangle no
# git changelog TAGNAME_FIRST TAGNAME_LAST
# #+end_src
...
#+begin_src conf
    ...
    mbase = !git show --summary $(git merge-base $(git origin)/$1 $(git current-branch))
...
#+begin_src conf
    ...
    export-commit = "!f() { git changed-files ${1:-HEAD} \
                                | tr '\\n' '\\0' \
                                | xargs -0 zip -9 $(git repo-slug)-commit-$(git current-branch)-${1:-HEAD}.zip; }; f"
#+end_src

XXX Put commit number instead of head
...
#+begin_src conf
    logdate = log --pretty='format:%ad %d %h %s %an%n' --date=short
