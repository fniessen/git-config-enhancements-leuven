...
#+begin_src conf
    export-commit = "!f() { git changed ${1:-HEAD} | xargs zip -9 $(git repo)-commit-${1:-HEAD}.zip; }; f"
#+end_src
...
#+begin_src conf
    repo = "!f() { git remote show origin -n | grep 'Fetch URL:' | sed -E 's#^.*/(.*)$#\\1#' | sed 's#.git$##'; }; f"
#+end_src
...
For the Github, such period of inactivity is *3 months* (see
...
#+begin_src conf
    # For GitHub.
    pr = "!f() { git fetch origin pull/$1/head:pr/$1 \
                     && git checkout pr/$1; }; f"
#+end_src
