...
Once you delete the branch from the remote, you can *prune remote-tracking
...
However, this command *doesn't remove local branches* that are already checked out
or that you've created locally.

*Remove orphaned branches in your local repository* that *no longer exist in the
remote* one (by deleting deleting any local branches that are marked as "gone"):

#+begin_src conf
    ##? Usage: git remove-gone-branches
    ##?
    ##? Delete local branches that are tracking remote branches that are gone.
    remove-gone-branches = "!f() { \
        git branch -vv \
            | grep ': gone]' \
            | awk '{print $1}' \
            | xargs git delete-local-branch; \
