...
#+begin_src conf
	in2 = pull --dry-run
	in = log --pretty=format:'%h %ad %s' --date=short HEAD..origin
#+end_src
...
The Incoming command will return a nicely formated list of incoming commits, the
...
The Outgoing command will return a nicely formated list of outgoing commits, the
next time you do a push.

Unpushed:

#+begin_src conf
	out = log --branches --not --remotes --color --pretty=format:'%C(yellow)%h%C(auto)%d%Creset %s %Cgreen<%an>%Creset %C(bold blue)(%ar)%Creset' --abbrev-commit
#+end_src

If you just want to see the outgoing commits for the *current branch*, you can use
the following:
...
#+begin_src conf
	unpushed2 = log --all --not --remotes --tags --pretty=format:'%h %an %s'
#+end_src
