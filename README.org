...
#+begin_src shell :tangle bin/git-update-local-branches :shebang #!/usr/bin/env bash
remotes="$@";
if [ -z "$remotes" ]; then
    remotes=$(git remote);
fi
...
This script handles more than just basic synchronization; it involves comparing
local and remote branches, performing fast-forward merges, and potentially
...
#+begin_warning
The follwaboveoing approach is *unsafe*: if there's nothing to stash, the first command
