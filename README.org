...
#+PROPERTY:  header-args :tangle .gitconfig_leuven :eval no
...
Welcome to git-config-enhancements-leuven!  This repository contains
...
If ~git~ is installed, an alias is created so you can use ~g~ in place of ~git~
on the command line.

#+begin_src shell :tangle bin/rc-common-settings-git-config-enhancements-leuven
...
If the ~zip~ command is not found, an error message prompts the user to install
it.
...
This alias changes the current directory to the root of the Git repository. It
uses ~git rev-parse --show-toplevel~ to determine the root of the repository and
ensures that no error is thrown if you aren't currently in a Git repository
directory.
...
To *test the above*, use ~git record~ or ~git revert~.
...
#+begin_src conf
[format]
    pretty = format:%C(green)%ad%C(reset) %C(yellow)%h%C(reset) %s%C(auto)%d%C(reset) %C(blue)<%an>%C(reset)
#+end_src
...
#+begin_src conf
    grep-aliases = "!git config -l \
            | grep '^alias\\.' \
            | cut -c 7- \
            | sort \
            | sed 's_=_ = _' \
            | sed 's/  */ /g' \
            | awk -F ' = ' '{printf \"\\033[30;47m%s\\033[0m = %s\\n\", $1, $2}' \
            | grep --color=auto -i"
#+end_src
...
#+begin_src conf :tangle no
    ...
    h = "!f() { \
        ...
        local ALIAS_NAME=\"$1\"; \
        local alias_description=\"$(git config alias.$ALIAS_NAME)\"; \
        if [ -z \"$alias_description\" ]; then \
            printf >&2 'Error: Alias not found\n'; \
            return 2; \
        fi; \
        local alias_help=\"$(git config alias.$ALIAS_NAME | sed -n '/^##?/,/^$/p')\"; \
    ...
    h = "!f() { \
        ...
        local ALIAS_NAME=\"$1\"; \
        local gitconfig_path=\"$(git root)/.git/config\"; \
        local alias_comments=\"$(awk -v alias=\"$ALIAS_NAME\" \
...
#+begin_src conf
    # Usage: git git-search SEARCH_TERM
...
| alias                       | removes ignored files | removes untracked files |
|-----------------------------+-----------------------+-------------------------|
| clean-ignored               | Yes                   | No                      |
| clean-ignored-and-untracked | Yes                   | Yes                     |

*** Delete only ignored files

How to remove ignored files (that are still on the repository)?
...
#+begin_src conf
    ##? Usage: git clean-ignored
    ##?
    ##? Remove only ignored files and directories.
    clean-ignored = clean -f -d -X
    # THIS COMMAND MAY DELETE FILES THAT CANNOT BE RECOVERED.
#+end_src

*WARNING:* These commands are irreversible and can lead to permanent data loss if
used carelessly. Proceed with caution.

**** Check before deleting ignored files

To preview what files and directories would be removed by ~git clean-ignored~
without actually deleting them, use:

: git clean-ignored-dry-run

#+begin_src conf
    ##? Usage: git clean-ignored-dry-run
    ##?
    ##? Display files and directories that would be removed by git clean-ignored.
    clean-ignored-dry-run = clean --dry-run -d -X
#+end_src

*** Delete ignored and untracked files

For a more aggressive cleanup that *removes both ignored and untracked files and
directories*, use:

: git clean-ignored-and-untracked
...
#+begin_src conf
    ##? Usage: git clean-ignored-and-untracked
    ##?
    ##? Remove both ignored and untracked files and directories.
    clean-ignored-and-untracked = clean -f -d -x
    # THIS COMMAND MAY DELETE FILES THAT CANNOT BE RECOVERED.
#+end_src
...
clean-ignored-and-untracked~ without actually deleting them, use:

: git clean-ignored-and-untracked-dry-run

#+begin_src conf
    ##? Usage: git clean-ignored-and-untracked-dry-run
    ##?
    ##? Display files and directories that would be removed by git
    ##? clean-ignored-and-untracked.
    clean-ignored-and-untracked-dry-run = clean --dry-run -d -x
#+end_src
...
git show 23231bf66 = git difftool 23231bf66^..23231bf66
...
#+begin_src conf
    ##? Usage: git search-commit-diffs REGEXP [PATH]
    ##?
    ...
    ##?
    ##? Examples:
    ##?   git search-commit-diffs "fix" "*.js"
    ##?   git search-commit-diffs "bugfix"
    search-commit-diffs = "!f() { \
        if [ -n \"$2\" ]; then \
            git log -p -G \"$1\" \"$2\"; \
        else \
            git log -p -G \"$1\"; \
...
The alias ~search-commit-diffs-all-branches~ searches for string-related changes
in the code changes (diff) across all branches.

#+begin_src conf
    ##? Usage: git search-commit-diffs-all-branches REGEXP
    ##?
...
#+begin_src conf
    ...
    amend-reuse = "!f() { \
        local upstream_commit=$(git upstream-commit 2> /dev/null); \
        local local_commit=$(git local-commit); \
...
#+begin_src conf
    ...
    squash = "!f() { \
        ...
        local NUM_OF_COMMITS_TO_UNDO=$1; \
...
The ~git bisect~ command is a powerful tool for identifying which commit
introduced a bug in your codebase. By performing a binary search through your
commit history, it efficiently narrows down the problematic commit.

*** Step-by-step guide to using git bisect

1. Start the bisect session

   Use the ~git bisect start~ command to begin. You need to specify a range
   ...
   Example with commit hashes:

   : git bisect start BAD-COMMIT GOOD-COMMIT

2. Mark a Commit as Bad

   Once the bisect session starts, mark the current commit as bad:

   : git bisect bad

3. Iterate Through Commits

   After marking commits as either bad (~git bisect bad~) or good (~git bisect
   good~), ~git bisect~ automatically checks out the next commit for
   testing. Repeat this process for each commit it suggests.

   Continue marking commits until ~git bisect~ isolates the first bad commit.

4. Identify the First Bad Commit

   When the faulty commit is found, ~git bisect~ outputs its hash:

   : git bisect bad
   : 2ddcca36c8bcfa251724fe342c8327451988be0d is the first bad commit

5. Inspect the Problematic Commit

   You can review the commit to understand what changes were introduced:

   : git show HEAD

6. Reset to Your Original State

   After identifying the bad commit, reset your repository to the branch you
   were on before starting git bisect:

   : git bisect reset

*** Practical Notes

- *Testing Each Commit:* Each time ~git bisect~ checks out a commit, you must test
  it to determine whether it is good or bad. This might involve running
  a script, performing manual testing, or executing a specific test suite.

- *Automating the Process:* For automated testing, use the ~--term~ options with
  ~git bisect~ to specify custom test scripts for marking commits as good or
  bad.

By using ~git bisect~, you can quickly pinpoint regressions and minimize time
spent troubleshooting complex histories.

*** Using sh -c with git bisect run

You can pass a series of commands to ~git bisect run~ using ~sh -c~. This is
particularly useful for automating the testing process during a bisect
session. Here's an example:

: git bisect run sh -c "make || exit 125; ./my_app | grep 'good output'"

- ~make || exit 125~: If the build fails, the exit 125 ensures git bisect skips
  the current commit as untestable.

- ~./my_app | grep 'good output'~: Runs your application and checks for specific
  output to determine if the commit is good or bad.

**** Simplifying Frequent Commands

If you find yourself using similar command sequences often, it's a good idea to
create a reusable script. For example:

1. Create a script file

   Save your commands in a script file (e.g., =test-script.sh=):
...
3. Use the script with ~git bisect run~
...
*** Using git bisect run to Find Performance Regressions

When investigating performance regressions, you can automate the process using
a script that identifies commits introducing performance issues. This script can
be passed to git bisect run:
...
# Build the application. Ignore build errors, as they are not the focus here.
make my_app || exit 255

# Run the application in the background to monitor its runtime behavior.
./my_app >log 2>&1 &

# Capture the process ID of the running application.
pid=$!

# Wait for a predefined normal runtime duration.
sleep $NORMAL_TIME

# Check if the application is still running.
if kill -0 $pid 2> /dev/null; then
    # The process is still running, indicating a regression.
    kill $pid; sleep 1; kill $pid
    exit 1
else
    # The process has completed within the expected time frame.
    exit 0
fi
#+end_src
...
1. Building the Application
   ...
   The script starts by building the application with ~make my_app~. If the build
   fails, the script exits with code ~255~, signaling ~git bisect~ to skip the
   commit as untestable.

2. Running the Application

   The application is executed in the background with ./my_app, and its output
   is redirected to a log file. The process ID (~pid~) is captured for monitoring.

3. Monitoring Runtime

   The script waits for a predefined period (~$NORMAL_TIME~) to simulate the
   expected runtime of the application.

4. Checking for Regressions
   + If the process is still running after the wait, the script identifies the
     commit as bad (~exit 1~).
   + If the process has completed, the commit is marked as good (~exit 0~).

*Using the Script with ~git bisect~*
...
This method helps identify commits that introduced performance issues by
analyzing application runtime behavior in an automated manner.
...
: git branch --edit-description --unset BRANCH
...
#+begin_src conf
    ...
    export-commit-files = "!f() { \
        local COMMIT_HASH=$(git rev-parse --short ${1:-HEAD}); \
        local current_branch=$(git convert-current-branch-name); \
        local zip_file_name=$(git repo-slug-remote)-commit-${current_branch}-${COMMIT_HASH}.zip; \
...
#+begin_src conf :tangle no
    issues-created-by-me = "!f() { \
        local REPO_OWNER=\"YourUsername\"; \
        local REPO_SLUG_REMOTE=\"YourRepository\"; \
        local API_URL=\"https://api.github.com/repos/$REPO_OWNER/$REPO_SLUG_REMOTE/issues?creator=$REPO_OWNER\"; \
...
#+begin_src conf
    # Lists all Git repositories by finding .git directories, sorting the
    # output, and removing the '/.git' suffix and leading './'.
    repos = !find . -type d -name '.git' -prune -print 2> /dev/null \
            | sed 's|^\\./||' | sed 's|/\\.git$||' | sort
    # FIXME: Local dir is outputted as '.git'...
#+end_src
...
#+begin_src conf :tangle no
    git show --color-words 23231bf66
#+end_src
...
#+begin_src conf
    ##? Usage: git ll
    ##?
    ##? Show medium formatted log with abbreviated commit hashes and file statistics.
...
#+begin_src conf
    ##? Usage: git merge-test BRANCH
    ##?
    ##? Test a merge and then abort it.
    merge-test = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git merge-test BRANCH\n'; \
            return 2; \
        fi; \
        local BRANCH=\"$1\"; \
        if [ -z \"$BRANCH\" ]; then \
...
- *Discard the Changes:* If the changes are not needed or you want to discard
  ...
  #+begin_src conf
    ##? Usage: git force-checkout BRANCH
    ##?
    ##? Force checkout to a branch, discarding local changes if necessary.
    force-checkout = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git force-checkout BRANCH\n'; \
            return 2; \
        fi; \
        local BRANCH=$1; \
        git checkout \"$BRANCH\" 2>&1 | { \
...
# Attempt to switch to the desired branch, while handling errors.
git checkout BRANCH 2>&1 | while read -r file_path; do
...
The command ~compare-remote-branch~ is useful for quickly identifying differences
between a local branch and its remote version. It helps determine which commits
...
#+begin_src conf
    ##? Usage: git compare-remote-branch [BRANCH_NAME]
    ##?
    ##? Show a visual comparison of the local branch against its remote
    ##? counterpart.
    compare-remote-branch = "!f() { \
...
[[https://github.com/fniessen/git-leuven/issues/new][GitHub issue tracker]].
...
If you find the "git-config-enhancements-leuven project" (or any of
...
development by making a [[https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=VCVAS6KPDQ4JC&lc=BE&item_number=git%2dleuven&currency_code=EUR&bn=PP%2dDonationsBF%3abtn_donate_LG%2egif%3aNonHosted][donation]] through PayPal. Your support is invaluable --
thank you!

Remember, regardless of donations, "git-config-enhancements-leuven project" will
