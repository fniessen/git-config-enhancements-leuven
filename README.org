...
#+begin_src conf
    ...
    search-for-code-in-extension = "!f() { \
        ...
        git local-branches | \
        while read -r branch; do \
            printf \"\n$(tput bold)$(tput setaf 2)Branch: $branch$(tput sgr0)\n\"; \
            git grep -n -- \"$PATTERN\" \"$branch\" -- \":*.$EXTENSION\"; \
        done; \
    }; f"
#+end_src
...
#+begin_src conf
    ...
    # Minimal, whitespace-insensitive diff (working dir vs index).
    mindf = diff --diff-algorithm=minimal -w
    mindf2 = diff -w --word-diff-regex=. --color-words -U0
#+end_src
...
#+begin_src conf
    ...
    # Minimal, whitespace-insensitive diff of stage changes (index vs HEAD).
    mindfc = diff --diff-algorithm=minimal -w --cached
#+end_src
...
#+begin_src conf
    ...
    squash-force = "!f() { \
        local NUM_OF_COMMITS_TO_UNDO=$1; \
        local COMMIT_MSG=$(git log --reverse --format=%B -n $NUM_OF_COMMITS_TO_UNDO); \
...
#+begin_src conf
    ...
    merge-pr = "!f() { \
        ...
        local USER=$(printf \"%s\" \"$USER_AND_REPO\" | cut -d' ' -f1); \
        local REPO=$(printf \"%s\" \"$USER_AND_REPO\" | cut -d' ' -f2); \
        ...
        local PULL_AUTHOR=$(awk '/\"head\":/{flag=1} flag&&/\"login\": \"/{print \\$2; exit}' \"$TMP_FILE\" | sed -E 's/\"([^\"]+)\".*/\\1/'); \
        local PULL_REF=$(awk '/\"head\":/{flag=1} flag&&/\"ref\": \"/{print \\$2; exit}' \"$TMP_FILE\" | sed -E 's/\"([^\"]+)\".*/\\1/'); \
        local PULL_TITLE=$(grep '\"title\": ' \"$TMP_FILE\" | sed 's/^ *\"title\": \"//' | sed 's/\",$//'); \
        rm -f \"$TMP_FILE\"; \
...
#+begin_src conf
    # Usage: git merge-dry-run BRANCH
    #
    # Check how the merge of BRANCH into master will go
    merge-dry-run = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git merge-dry-run BRANCH\n'; \
            return 64; \
        fi; \
