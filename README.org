...
#+begin_src conf
    ...
    pager = "!f() { if command -v diff-so-fancy &>/dev/null; then echo 'Using diff-so-fancy pager' && diff-so-fancy | LESS=less --tabs=4 -FRX \"$@\"; else echo 'Using default pager' && LESS=less --tabs=4 -FRX \"$@\"; fi; }; f"
...
#+begin_src conf :tangle no
    ...
    h = "!f() { \
        ...
        local alias_name=\"$1\"; \
        local gitconfig_path=\"$(git rev-parse --show-toplevel)/.git/config\"; \
...
#+begin_src conf
    ...
    ##? Usage: git undo-changes [FILE]
    ##?
    ##? Revert changes for a specific file or all files if no file is provided.
...
: git status | grep deleted | sed 's_deleted:__g' | sed 's_[#| ]__g' | xargs git rm
...
#+begin_src conf
    ...
    check-remote-branch = "!f() { \
        ...
        if [ -z \"$branch_name\" ]; then \
            echo \"Usage: git check-remote-branch BRANCH_NAME\"; \
        else \
    ...
    all-repos = "!f() { \
        ...
        find . -type d -name '.git' \
            ...
            | while read -r repo_path; do \
                ...
                git -C $repo_path -c color.status=always \"$@\" 2>&1 | sed 's/^/  /'; \
                printf '\n\n'; \
            done; \
    }; f"
#+end_src
...
#+begin_src conf \
    ...
    file-history2 = "!f() { \
        ...
        FILENAME=\"$1\"; \
        git log --oneline --graph --decorate --follow -- \"$FILENAME\"; \
    }; f"
    ...
    file-history-all = "!f() { \
        ...
        FILENAME=\"$1\"; \
        git log --oneline --graph --decorate --follow --all -- \"$FILENAME\"; \
    }; f"
    ...
    file-history-all2 = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 \"Usage: git file-history-all2 FILENAME\n\"; \
            return 2; \
        fi; \
        FILENAME=\"$1\"; \
        git log --pretty=format:'%h %ad %an %s' --date=short --graph --decorate --follow --all -- \"$FILENAME\"; \
    }; f"
#+end_src
