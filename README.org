...
#+begin_src conf
    ##? Usage: git search-commit-diffs SEARCH_STRING PATH
    ##?
    ##? Search Git log for a specific string and show the patch. The path can be
    ##? a file, directory, or file pattern (e.g., *.txt).
    ##? Only shows patches where the number of added instances of the string is
    ##? different from the number of deleted instances.
    ##?
    ##? Example:
...
- ~-G~: Matches lines in the content of files that contain the specified pattern.
- ~-S~: Matches exact occurrences of the specified string in the content of files.

***** Behavior:

- ~-G~: Useful when you want to track changes related to the presence or absence
  of a pattern (such as keywords, TODOs, etc.) in the files.
- ~-S~: Useful when you are interested in changes that specifically involve the
  addition or removal of instances of an exact string in the files.

***** Usage Considerations:

- Choose ~-G~ when you need to track changes based on patterns that may appear in
  multiple contexts.
- Choose ~-S~ when you need to track changes based on exact strings where the
  context and exact occurrences are critical.

***** Example:

To illustrate the difference further:

- If you have a file where the lines containing "TODO" were added or removed,
  you would use ~-G "TODO"~.
- If you have a file where the exact occurrences of the string "functionName"
  were added or removed, you would use ~-S "functionName"~.

Understanding these distinctions helps you choose the appropriate option (~-G~
or ~-S~) based on what you're trying to track in your Git history.
...
#+begin_src conf
    ...
    amend-reuse = "!f() { \
        local upstream=$(git upstream-commit 2>/dev/null); \
        local local_commit=$(git local-commit); \
        if [ -n \"$upstream\" ] && [ \"$upstream\" = \"$local_commit\" ]; then \
    ...
    force-checkout = "!f() { \
        ...
        git checkout \"$BRANCH\" 2>&1 | { \
            while IFS= read -r line; do \
                if printf '%s\n' \"$line\" | grep -q 'Your local changes to the following files would be overwritten'; then \
                    continue; \
                fi; \
                # Remove leading spaces from file_path. \
                file_path=$(printf '%s\n' \"$line\" | sed 's/^[[:space:]]*//') \
