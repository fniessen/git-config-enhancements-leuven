...
#+begin_src conf
[init]
    # Set the default branch for new repositories to master.
...
#+begin_src conf
[log]
    # Display the date in a short format.
    date = short
#+end_src
...
#+begin_src conf
    # Display ref names (e.g. branch or tag names) in a short format.
    decorate = short
#+end_src
...
#+begin_src conf
    # Enable dimmed zebra coloring to highlight moved lines in a diff.
...
#+begin_src conf
    # Usage: git start-daemon
    # Start a Git daemon server for remote repository access.
    start-daemon = "-c daemon.receivepack=true daemon --base-path=. --export-all --reuseaddr --verbose"
#+end_src

1. Run ~git start-daemon~. "Ready to rumble," it will report. Git is bad-ass.
...
3. Say "hey Jane, I'm not ready/able to push these commits up to origin, but you
   ...
   git://192.168.1.123/"
4. Press C-c when you don't want to serve that repo any longer.
...
#+begin_src conf
    ...
    # Show files that have been added but not yet committed.
    added-files = !git status --porcelain -uno | awk 'match($1, /A/) {print $2}'
#+end_src

#+begin_src conf
    # Usage: git modified-files
    # List all files modified in the working tree.
    modified-files = !git diff --name-only --diff-filter=M
    #+end_src
...
#+begin_src conf
    ...
    untracked-files2 = !git status --porcelain -uno | awk 'match($1, /\\?/) {print $2}'
#+end_src
...
#+begin_src conf
    ...
    # Show deleted files in the working directory.
    deleted-files = ls-files -d
#+end_src

How to restore a deleted file in a Git repository?

: git deleted | xargs git checkout --

#+begin_src conf
    # Usage: git staged-files
    # Show a list of staged added, modified or deleted files.
    staged-files = !git status --porcelain -uno | grep -P '^[ADM]' | awk '{ print $2 }'
#+end_src

*** Tell if a file is being tracked

#+begin_src conf
    # Usage: git file-status FILENAME
...
#+begin_src conf
    ...
    where3 = for-each-ref --format="%(refname:short)" --points-at HEAD refs/heads
#+end_src
...
#+begin_src conf
    # Usage: git whatwhen STRING
    # Search for changes that introduced or removed a string.
    whatwhen = log -p -S
#+end_src
...
#+begin_src conf
    # Usage: git search-for-diff-contents STRING
    # Search for commits that add or remove a specific string in the repository.
...
#+begin_src conf
    # Usage: git grep4d STRING FILENAME
    # Show commits that add or remove a specific string in a file.
    # Example: git grep4d CLOSED '*.java'
...
#+begin_src conf :tangle no
    # Squash changes made in multiple unpushed commits into a single commit for
    # a polished and cohesive project history
    squash-BUGGY = "!f() { \
                [ $(git rev-list HEAD..origin/$(git rev-parse --abbrev-ref HEAD) --count) -eq 0 ] \
                    && [ $1 -le $(git rev-list --count HEAD) ] \
                    && git reset --soft HEAD~$1 \
                    && git commit --edit -m\"$(git log --reverse --format=%B HEAD..HEAD@{1})\" \
                    || echo 'Error: Cannot squash already pushed commits or squash more commits than unpushed'; \
            }; f"
#+end_src
...
#+begin_src conf
    ...
    d = !"git diff-index --quiet HEAD -- || clear; \
...
#+begin_src conf
    ...
    precommit0 = diff --diff-algorithm=minimal -w
#+end_src
...
#+begin_src conf
    precommit = diff --diff-algorithm=minimal -w --cached
#+end_src
...
#+begin_src conf
    ...
    # Create a temporary stash before pulling, and apply it after the pull.
