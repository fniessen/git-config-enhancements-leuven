...
If unable to clone Git repository due to self signed certificate, disable Git
SSL verification:
...
*** Search regexp within commit messages

You can find commits that contain particular words in the message. For example,
fix typo matches commits containing the words "fix" and "typo."
...
#+begin_src conf
    pull-all = !"old=$(git rev-parse --abbrev-ref HEAD); \
...
#+begin_src conf
    ...
    br-date = !"sh -c 'for b in $(git branch --no-merged); do \
                           git logdate -1 -r $b --; \
                       done'"
#+end_src
...
Find file (by path) in the current branch.

#+begin_src conf
    find-file = !git ls-tree -r --name-only $(git branch-name) | grep --color=auto
#+end_src
