...
- grep  :: search with basic regular expressions (BRE)
- egrep :: search with extended regular expressions (ERE)
- fgrep :: find string
...
#+begin_src conf
    ...
    # Append staged changes to the PREVIOUS commit (changes its SHA1, keeps the commit message).
...
~delete-merged-branches~ *removes* (in bulk) *all* the *branches* that have been *merged
to the current branch* that you are in.

#+begin_src conf
    # Remove branches that have already been merged with master.
    delete-merged-branches = "!git branch --no-color --merged | grep -v '\\*' | grep -v master | xargs -n 1 git branch -d"
#+end_src
...
3. Delete all branches already merged into =master= using:
   ~git delete-merged-branches~

#+begin_src conf
    bdone = "!f() { git checkout ${1-master} && git up && git delete-merged-branches; }; f"
#+end_src
...
and right side commit SHA1s (as obtained from the names, or defaulting to =HEAD=)
and substitutes that in for the left-side SHA1. The right-side SHA1 (as resolved
...
Alias configuration for our topic branches, like a simple git flow for feature
...
If your team uses a different topic flow, you may want to skip including these
aliases, or you may want to edit these aliases to match your team's topic flow.
...
Use ~git push -u origin <current-branch>~ to push the ~<current-branch>~ branch to
the =origin= remote and to set up tracking.

#+begin_src conf
    # Get the current branch name (not so useful in itself, but used in other
    # aliases).  Works well in detached mode.
    branch-name = name-rev --name-only HEAD
    # branch-name = !git branch --show-current # v2.22+
#+end_src
...
#+begin_src conf
    # repo-user
    # repo-name
    changelog-github-md  = !sh -c 'git log $1...$2 --pretty=format:\"  * %s [[http://github.com/$3/$4/commit/%H][view commit]]\"' -
    changelog-github-org = !sh -c 'git log $1...$2 --pretty=format:\"- %s [[http://github.com/$3/$4/commit/%H][view commit]]\"' -
#+end_src

Use it like this:

#+begin_src shell :tangle no
git changelog-github-org first_tag last_tag github_user github_repo
#+end_src
...
#+begin_src conf
    ...
    mbase = !git show --summary $(git merge-base origin/$1 $(git branch-name))
#+end_src

#+begin_src conf
  # # Deploy
  # staging = !git push heroku-staging $(git branch-name):master -f && git track 'Deployed to staging ' # push current branch to staging, can add more comments after
  # dev = !git push heroku-dev $(git branch-name):master -f && git track 'Deployed to dev ' # push current branch to dev
  # deploy = !git push heroku $(git branch-name):master -f && git track 'Deployed to production ' # push current branch to production


 # deploy='git push production master && terminal-notifier-notify -title "Deploy" -message "Deploy complete"'
 # stage='git push staging $(git branch-name):master'
#+end_src
...
current tag. Its been useful for uploading to Elasticbeanstalk
...
alias nxtrlz='git log --oneline $(git lasttag)..HEAD && git lasttag'
...
- *annotated tags* in /Bitbucket/, and
- *lightweight tags* via /GitHub/ interface (for releases).

Git stores annotated tags as full objects in the repository, which means they
include the name, email, date, and have a message.

Lightweight tags don't include all this additional information.
...
* Compare Revisions (branches, tags or commits)

*See what's changed*: easily determine which commits are on the source but not on
the destination.

You can see:
- a diff,
- a list of commits, and
...
: git diff --stat origin/master...my_forked_branch
    ...
    #+begin_src conf
    # Show the names of all files changed between two commits.  Usage: "git diffn SHA1v1 SHA1v2".
