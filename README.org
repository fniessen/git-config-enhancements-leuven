...
#+begin_src conf
    ...
    feature-finish = "!f() { git checkout develop; \
                             git merge --no-ff feature/$1 && git delete-local-branch feature/$1; }; f"
...
Once you delete the branch from the remote, you can remove all tracking branches
that you have locally but are no more present in the remote repository (~origin~)
with:
...
Check out a pull request locally (for GitHub project):

#+begin_src conf
    pr1 = '!sh -c "git fetch origin pull/$1/head:pr/$1" -'
    pr2 = "!f() { git fetch -fu origin refs/pull/$1/head:pr/$1 && \
                 git checkout pr/$1; }; f"
#+end_src
...
** Search from which PR comes a commit
...
#+begin_src conf
    list-issues = "!f() { username=$1; repo_slug=$2; \
        curl --user $username \
            https://api.bitbucket.org/2.0/repositories/$username/$repo_slug/issues \
            | jq --compact-output '.values | .[] | [.id, .title, .type, .priority, .state, .votes, .assignee.display_name, .created_on, .updated_on]'; }; f"
# --data-urlencode 'priority >= \"major\"'
#+end_src

#+begin_src conf :tangle no
# Works!  Need to put quotes around the URL!
curl -u $username -X GET 'https://api.bitbucket.org/2.0/repositories/mcpflow/pflow/issues?pagelen=100' \
...
You can search through *All*, *Open* or *My* issues.
...
#+begin_src conf
    open-issues = '!f() { URL=$(git config --get remote.origin.url); open ${URL/.git}/issues/$1; }; f'
#+end_src
...
#+begin_src conf
    ...
    download-repository = "!f() { git archive ${1:-HEAD} --format=zip -o $(basename $(git root))-commit-$(git rev-parse ${1:-HEAD}).zip; }; f"
#+end_src
