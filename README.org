...
List all the changed files *between 2 commits*:
    ...
    #+begin_src conf
    # Show the names of all files changed between two commits.  Usage: "git dname SHA1 SHA2".
...
# good name???  Is this what it does?
#+end_src
...
#+begin_src conf
    snapshot = !git stash push "Snapshot [$(date +\"%F %a %R\")]" && git stash apply "stash@{0}"
#+end_src

#+begin_src conf
    # checkpoint and checkpoint recover
    cp      = !git stash push $(date +%F--%T) && git stash pop --index
