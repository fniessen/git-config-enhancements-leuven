...
#+begin_src conf
    ...
    search-code = "!f() { git log --decorate --date=short -S $1; }; f"
...
: topic-pull = '!branch=$(git current-branch); git checkout master; git pull; git checkout "$branch"; git rebase master'

Share the topic branch:

: topic-push = '!branch=$(git current-branch); git push -u origin "$branch"'
