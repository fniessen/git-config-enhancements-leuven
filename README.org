...
The alias ~search-for-diff-contents~ searches for string-related changes:
...
It searches for changes related to a specific string in diffs only?

or

It searches for changes related to a specific string in commit messages, file
changes, and diffs?
...
#+begin_src conf
    ...
    search-changes-for-term = log --all -p -S
#+end_src
...
#+begin_src conf
    ...
    wwflu = "!f() { \
        local_branches=$(git rev-parse --symbolic --branches); \
