...
#+begin_src conf
    ...
    find-branches-containing = "!f() { \
        if [ $# -ne 1 ]; then \
            printf 'Usage: git find-branches-containing COMMIT_HASH\n'; \
            return 2; \
        fi; \
        local commit_hash=\"$1\"; \
...
*** Iterating over Git commits

You can use the ~git-iterate-commits~ function to iterate over Git commit hashes
(obtained from the previous command) and execute a specified Git command. This
can be particularly beneficial when you need to perform operations on each
commit within a series.

**** Example

Let's say you want to execute the ~changed-files~ Git command for each commit
reported by:
...
#+begin_src conf
    ...
    has-new-commits = "!f() { \
        ...
        [ $(git rev-list --count origin/\"$branch\"..\"$branch\") -gt 0 ]; \
    }; f"
#+end_src
            ...
            if git check-remote-branch \"$branch\"; then \
                if git is-ancestor origin/$branch $branch; then \
                    ...
                    if git pull origin $branch:$branch; then \
                        ...
                        printf \"\\e[91mError: Pull failed for branch \\e[1m%s\\e[0m.\n\\e[0m\" \"$branch\"; \
                        if git rebase --abort > /dev/null 2>&1; then \
                            printf \"Rebase operation aborted.\n\n\"; \
                        else \
                            printf \"\\e[91mCould not abort the rebase operation. Please do so manually.\n\\e[0m\"; \
                        fi; \
                        continue; \
                    fi; \
                else \
...
#+begin_src conf \
    ...
    file-history = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git file-history FILENAME\n'; \
            return 2; \
        fi; \
        FILENAME=\"$1\"; \
        if ! git rev-parse --quiet --verify \"$FILENAME\" 2>/dev/null; then \
...
#+begin_src conf
    ...
    ##?
    ##? Show the commit history of a specific file across all branches.
    file-history-all = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git file-history-all FILENAME\n'; \
            return 2; \
        fi; \
        FILENAME=\"$1\"; \
        if ! git rev-parse --quiet --verify \"$FILENAME\" 2>/dev/null; then \
...
#+begin_src conf
    ##? Usage: git file-most-recent-commits FILE_PATH
    ##?
    ##? Show the most recent commits for a specific file across all local branches.
    file-most-recent-commits = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git file-most-recent-commits FILE_PATH\n'; \
            return 2; \
        fi; \
        local file_path=\"$1\"; \
        local local_branches=$(git local-branches); \
    ...
    # Alias of 'file-most-recent-commits'.
    wwflu = file-most-recent-commits
#+end_src

: git file-most-recent-commits schema/ab-products/bldgops/common/console/ab-bldgops-console-wr-ds.axvw

#+begin_src conf
    file-most-recent-commits2 = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git file-most-recent-commits2 FILE_PATH\n'; \
            return 2; \
        fi; \
        local file_path=\"$1\"; \
...
#+begin_src conf
    ##? Usage: git dry-merge BRANCH_NAME
    ##?
    ##? Simulate a "dry run" merge of a branch into the current branch.
    ##?
    ##? Example:
    ##?   git dry-merge feature # Merge the `feature` branch without committing the changes.
    dry-merge1 = "!f() { \
        if [ $# -ne 1 ]; then \
