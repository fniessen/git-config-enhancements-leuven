...
#+begin_src conf
    today = !git log --since=midnight --author=\"$(git config user.name)\" --no-merges --all --pretty='%ad <<<DEL %s' --date=format:'%Y-%m-%d %a %H:%M'
    yesterday = !git log --since='1 day ago midnight' --until=midnight --author=\"$(git config user.name)\" --no-merges --all --pretty='%ad <<<DEL %s' --date=format:'%Y-%m-%d %a %H:%M'
    this-week = !git log --since=1.weeks --author=\"$(git config user.name)\" --no-merges --all --pretty='%ad - %s' --date=format:'%Y-%m-%d %a %H:%M'
#+end_src
...
#+begin_src conf
    ll = log --pretty=format:"%C(yellow)%h\\ %C(green)%ad%C(auto)%d\\ %C(reset)%s%C(blue)\\ <%cn>" --date=short --stat
#+end_src
