...
To delete all files (and directories) unknown to Git, you can run ~git purge~.
...
To check what ~git purge~ will delete, and be sure it's what you want, use ~git
purge-dry-run~.

#+begin_src conf
    # Usage: git purge-dry-run
    # Show files to delete instead of deleting them.
    purge-dry-run = clean -d -n

    # Alias to purge-dry-run.
    clean-tree-unknown-dry-run = !git purge-dry-run
#+end_src
...
#+begin_src conf
    ...
    purge-all = clean -d -f -x
    # THIS COMMAND MAY DELETE FILES THAT CANNOT BE RECOVERED,
    # including those in .gitignore.

    # Alias to purge-all.
    clean-tree-unknown-ignored = !git purge-all
#+end_src
