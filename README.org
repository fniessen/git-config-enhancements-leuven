...
#+begin_src conf
    ...
    origin = !echo "origin"
    # git remote get-url origin | sed 's_\.git$__'
#+end_src
...
#+begin_src conf
    ...
    # Note: The alias
    # main-branch = "!git symbolic-ref refs/remotes/origin/HEAD \
    #     | sed 's_^refs/remotes/origin/__'"
...
: echo "# ${$(git repo-slug-remote):-Repository name}\n\nDescription" >> README.md
: git add README.md
...
#+begin_src conf
    ...
    ##?
    ##? Arguments:
    ##?   FILE_NAME                     The name of the file to search for in all local branches.
    ...
    # Alias for 'search-for-files-all'.
...
List *all* of the *files* in the working directory that are *not tracked* by Git.
It's useful for quickly identifying all files that Git does not currently
manage.
...
List untracked *files* that are *ignored* according to your Git repository's
=.gitignore= rules and patterns:
...
If you wish to safeguard the current state of your current branch, you can
...
#+begin_src conf
    ...
    file-diff-last-modification = "!f() { \
        local project_root_dir=$(git root); \
        echo Finding full file path of \"$1\" in $project_root_dir; \
        local file_path=$(find $project_root_dir -type f -name \"$1\"); \
    ...
    file-diff-last-modification2 = "!f() { \
        ...
        echo Full file path $file_path; \
        local last_modified_commit_hash=$(git log -1 --pretty=format:%H -- $file_path); \
        echo Last commit file modified $last_modified_commit_hash; \
        git difftool $last_modified_commit_hash^ $file_path; \
    }; f"
#+end_src
...
This command lists all branches (both local and remote) that contain the
...
This includes *remote tracking branches*, which are local branches that have
a direct relationship to a remote branch.
...
#+begin_src conf
    ##? Usage: git search-in-all-commits SEARCH_TERM
    ##?
    ##? Search for a specific term in all commits.
    search-in-all-commits = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git search-in-all-commits SEARCH_TERM\n'; \
            return 2; \
        fi; \
        local SEARCH_TERM=\"$1\"; \
...
#+begin_src conf
    # Usage: git search-for-commits-or-bitbucket KEYWORD...
    #
    # Search for commit messages that match one or more search patterns (OR semantic).
    #
    # This will search commit messages for the provided keywords and list the
    # matching commits.
    search-for-commits-or-bitbucket = "!f() { \
        for keyword in \"$@\"; do \
            git log --grep \"$keyword\" -i --format=\"%at:%C(green)%ad%C(reset) %C(yellow)%h%C(auto)%d%C(reset) %s %C(bold blue)<%an>%C(reset)\" --color=always; \
            printf \"\n\"; \
        done | sort -r; \
    }; f"
    # XXX Note: Sorting may not be accurate if all commits have the same timestamp.
...
#+begin_src conf
    # Usage: git search-for-commits-and-gitlab PATTERN...
    #
    # Search for commits that match all provided search patterns in commit messages (AND semantic).
    #
...
**** Search for commits that introduced or removed instances of a specific string

The alias ~search-changes-in-message-and-diff~ searches for string-related changes
in the commit message and the diff.
...
#+begin_src conf :tangle no
    ...
    fco="!f() { \
        git branch -a -vv --color=always --format='%(refname)' \
            | sed "s_refs/heads/__" \
            | sed "s_refs/remotes/__" \
            | fzf --query="$@" --height=40% --ansi --tac --color=16 --border \
            | awk '{print $1}' \
            | xargs git checkout; \
    }; f"
#+end_src
...
#+begin_src conf
    # Usage: git check-remote-branch BRANCH_NAME
    # Check if a remote branch exists.
    check-remote-branch = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git check-remote-branch BRANCH_NAME\n'; \
            return 2; \
        fi; \
        local BRANCH_NAME=\"$1\"; \
...
#+begin_src conf
    ...
    has-new-commits = "!f() { \
        ...
        [ $(git rev-list --count origin/\"$BRANCH_NAME\"..\"$BRANCH_NAME\") -gt 0 ]; \
    }; f"
#+end_src
...
#+begin_src conf
    ...
    update-all-branches2 = "!f() { \
        ...
        for branch in $local_branches; do \
            ...
            if git check-remote-branch \"$branch\"; then \
                if git is-ancestor origin/$branch $branch; then \
                    printf \"No conflicts found in \\e[1m%s\\e[0m. Updating...\n\" \"$branch\"; \
                    if git pull origin $branch:$branch; then \
                        printf \"\n\"; \
                    else \
...
#+begin_src conf
    ##? Usage: git history-graph
    ##?
    ##? Display a commit history graph.
    history-graph = log --graph

    ##? Usage: git history-graph2
    ##?
    ##? Display a commit history graph with branches, tags, and remotes.
    history-graph2 = log --branches --tags --remotes --date-order --format=format:'%C(yellow)%h %C(reset) %<(75,trunc)%s <%cn> (%cr) %C(red)%d' --graph

    ##? Usage: git history-all-graph
    ##?
    ##? Display a complete commit history with a graph.
    history-all-graph = log --all --graph

    ##? Usage: git history-details
    ##?
    ##? Display a detailed commit history graph with file changes.
    history-details = log --pretty=format:'%C(green)%cr%C(reset) %C(yellow)%h%Creset %C(yellow)%d%Creset %s %C(bold blue)<%an>%Creset%n' --graph --name-status
#+end_src
...
#+begin_src conf
    ...
    export-commit-files = "!f() { \
        ...
        local current_branch=$(git convert-current-branch-name); \
        local commit_hash=$(git commit-hash); \
        local zip_file_name=$(git repo-slug-remote)-commit-${branch_name}-${target_commit:-$commit_hash}.zip; \
        git changed-files \"$target_commit\" | tr '\n' '\\0' | xargs -0 zip -9 \"$zip_file_name\"; \
    }; f"
#+end_src
...
#+begin_src conf
    ...
    ##?
    ##? Run a Git command for each repository in the current directory and its subdirectories.
    ##?
    ##? Arguments:
    ##?   COMMAND                       Git command to be executed in each repository
    ##?   ARGS                          Additional arguments to be passed to the Git command
    all-repos = "!f() { \
        if [ $# -lt 1 ]; then \
...
#+begin_src conf
    ...
    diff-commits = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git diff-commits NUMBER\n'; \
            return 2; \
        fi; \
...
# Tests:
# : git file-history-all db/aremis/Sprint_2024_07/GCS-4307_2_BBS_ROLES_roles.sql
# : git file-history-all db/aremis/Sprint_2024_07/GCS-4307_BBS_ROLES_FOR_S2_STACK-2.sql
...
#+begin_src conf
    # Usage: git file-last-commit FILENAME
    # Show the most recent commit across all branches (both local and remote).
    file-last-commit = log -n 1 --all --date-order
#+end_src
  ...
  git checkout BRANCH 2>&1 | {
      ...
      while IFS= read -r FILE_PATH; do
          ...
          if [ -n "$FILE_PATH" ]; then
              ...
              # Discard changes for the blocked file.
...
TODO: Make author be case insensitive (with regexp and ignore case?)
...
Your =.gitconfig= file might resemble the following configuration:
