...
#+begin_src conf
    findc1 = "log --decorate -i --all --grep"
    findc2 = "!f() { \
            git log --decorate --grep=$1 -i --all; \
        }; f"
#+end_src
...
#+begin_src conf
    ...
    search-for-author = "!f() { \
            git log --decorate -i --no-merges --all --author=${1-$(git config user.email)}; \
        }; f"
#+end_src
...
#+begin_src conf
    # Delete all branches that were safely merged into the master. All other are skipped (no worries).
...
: git delete-local-branchr BRANCH_NAME
...
#+begin_src conf
    ll = log --pretty=medium --abbrev-commit --decorate --stat
#+end_src
...
#+begin_src conf
    ...
    incoming = !(git remote update -p && git log --decorate ..@{u})
...
#WHAT WAS GOING ON, WHILE YOU WERE AWAY
    ...
    incoming2 = !sh -c 'git fetch' && git log --oneline HEAD..$(git origin)/$1
    incoming2onmaster = !sh -c 'git fetch' && git log --oneline HEAD..$(git origin)/master
#+end_src
...
: feature-start = '!BRANCH_NAME=$1; git checkout master; git pull; git create-branch "$BRANCH_NAME" master'

Update the feature branch:

: feature-pull = '!BRANCH_NAME=$(git current-branch); git checkout master; git pull; git checkout "$BRANCH_NAME"; git rebase master'

Share the feature branch:

: feature-push = '!BRANCH_NAME=$(git current-branch); git push -u $(git origin) "$BRANCH_NAME"'
...
#+begin_src conf
    ...
    outgoing-all = log --branches --not --remotes --decorate
...
#+begin_src conf
    ...
    zopa = !git log --decorate --all --pretty=oneline --abbrev-commit --graph \
