...
#+begin_src conf \
    ##? Usage: git file-history PATH
    ##?
    ##? Show the commit history of a specific file or directory.
    file-history = "!f() { \
        if [ $# -ne 1 ]; then \
            printf >&2 'Usage: git file-history PATH\n'; \
            return 2; \
        fi; \
        PATH=\"$1\"; \
        if ! git rev-parse --quiet --verify \"$PATH\" 2>/dev/null; then \
