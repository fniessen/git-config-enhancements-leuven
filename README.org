...
- ~master~ or ~develop~ (used as integration branch for feature work)
- ~master~ or ~production~ (used for deploying a release)
- ~bugfix/~ (typically used to fix Release branches, such as Jira *bugs*)
- ~feature/MSW-709-add-support-for-webm~ (for new features, such as Jira *stories*)
- ~hotfix/fix-123-for-production~ (used to urgently fix a Production branch)
- ~release/2018-12-24~ (used for preparing and testing release tasks)
#+end_tip
