...
**** Soft option -- Clean branch (delete the untracked files)
...
**** Hard option -- Clean branch (delete the ignored files in addition to the untracked files)
...
*See what's changed*.  Easily determine which changes:
...
: git log --date=short <rev-base>...<rev-compare>
...
: git log --stat <rev-base>...<rev-compare>
: git diff --stat <rev-base>...<rev-compare>
...
#+begin_src conf
    # Show the names of all files changed between two commits.  Usage: "git diffn <rev1> <rev2>".
