...
*** Diff of branch since it forked from another branch, or since the last merge

To see the changes in a branch since it was forked from another branch, or
since the last merge with the origin branch, you can use:

: git diff forked_from_branch..forked_branch --stat

Example:

: git diff origin/master..my_forked_branch --stat
...
List all the changed files *between 2 commits*:
    ...
    #+begin_src conf
    # Show the names of all files changed between two commits.  Usage: "git diffn SHA1 SHA2".
...
#+begin_src conf
    ...
    # Append staged changes to the PREVIOUS commit (changes its SHA-1, keeps the commit message).
...
#+begin_src conf
    release = '!git commit -am "Release $1" && git tag -a -m $1'
#+end_src
