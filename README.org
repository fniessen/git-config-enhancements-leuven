...
#+begin_src conf
    ...
    current-branch-without-slash = "!f() { git current-branch | tr '/' '-'; }; f"
#+end_src
...
#+begin_src conf
    # Usage: git missing-master [COMMIT_ISH]
    # Show changed files since master.
    # Examples:
    #   git missing-master            list changed files for the current branch
    #   git missing-master feature/7  list changed files for the feature/7 branch
    missing-master = "!f() { git diff --stat master...$1; }; f"
#+end_src
...
copies, and shows the merge commits!):

#+begin_src conf
    file-history = log --follow --date-order --date=short -C -m --
#+end_src
...
#+begin_src conf
    file-last-modified = log -n 1 --follow --all --date-order --date=short -C -m --
#+end_src
...
#+begin_src conf
    ...
    export-commit = "!f() { git changed-files ${1:-HEAD} \
                                | tr '\\n' '\\0' \
                                | xargs -0 zip -9 $(git repo-slug)-commit-$(git current-branch-without-slash)-${1:-$(git commit-hash)}.zip; }; f"
#+end_src
