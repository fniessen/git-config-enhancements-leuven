...
This command deletes all files and directories listed in =.gitignore=, while
leaving *untracked files untouched*. Staged changes, such as modified but
uncommitted files, are also preserved.
...
: git clean-ignored-dry

#+begin_src conf
    # Usage: git clean-ignored-dry
    # Display files and directories that would be removed by git clean-ignored.
    clean-ignored-dry = clean -n -d -X
#+end_src
...
This command is especially dangerous because it removes files and directories
that are not under Git tracking. Be cautious as *deleted files cannot be
recovered*.
...
To preview what files and directories would be removed by ~git clean-all~ without
actually deleting them, use:
...
*Note:* Always use the ~-dry~ variants of the commands first to verify which files
will be deleted.
