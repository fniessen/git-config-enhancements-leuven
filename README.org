...
To *remove only ignored files and directories*, you can run ~git clean-ignored~.

Only ignored files (and directories) -- not unknown files -- are deleted: it
*leaves all untracked files untouched*.
...
To check what ~git clean-ignored~ will delete, and be sure it's what you want, use
~git clean-ignored-dry~.

#+begin_src conf
    # Usage: git clean-ignored-dry
    # Show what files and directories would be removed by the clean-ignored
    # alias without actually removing them.
    clean-ignored-dry = clean -n -d -X
#+end_src

*** Delete all ignored files and also all untracked files

*PLAY WITH FIRE VERSION* which will wipe out *all ignored and untracked files and
directories*:
