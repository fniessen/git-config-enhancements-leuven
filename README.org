...
Easily launch a temporary one-off Git server from any local repository, to
enable a peer-to-peer Git workflow.
...
clean-tree-unknown~.  Only unknown files, not ignored files, are deleted.
...
To check what ~git clean-tree-unknown~ will delete, and be sure it's what you
want, use ~git clean-tree-unknown-dry-run~.
...
To check what ~git clean-tree-unknown-ignored~ will delete, and be sure it's what
you want, use ~git clean-tree-unknown-ignored-dry-run~.
...
#? git-is-ancestor, Time-stamp: <<current-time()>>
...
Combine together a change done in several commits into one commit so that people
think you are smart.
...
#+begin_src conf
    recent1 = for-each-ref \
                --sort=committerdate refs/heads/ \
                --format='%(HEAD) %(color:yellow)%(refname:short)%(color:reset) - %(color:red)%(objectname:short)%(color:reset) - %(contents:subject) - %(authorname) (%(color:green)%(committerdate:relative)%(color:reset))'
#+end_src

#+begin_src conf
    recent2 = for-each-ref \
                --sort=-committerdate refs/heads/ \
...
#+begin_src conf
    ...
    showorigin = "!sh -c 'for BRANCH_NAME in $(git branch -r | grep -v HEAD); do echo $(git show -s --format=\"%Cred%ci %C(green)%h %C(yellow)%cr %C(magenta)%an %C(blue)\" $BRANCH_NAME | head -n 1) \\\t$BRANCH_NAME; done | sort -r'"
...
: git log --graph --left-right --cherry-pick --oneline master...BRANCH_NAME
...
forgetting anything, I run ~git bulk status --short~ to quickly scan which repos
...
#+begin_src conf
    bulk-status = "!for D in $(find . -name \".git\"); do \
                      echo \"\n*** Repository: $D ***\" \
                          && git --git-dir=$D --work-tree=$D/.. status | grep -q \"is ahead of\"; \
                    done"
#+end_src

** Git-bulk-pull and git-bulk-push
...
#+begin_src conf
    ...
    deleted   = "!git status --porcelain -uno | awk 'match($1, /D/) {print $2}'"
#+end_src
...
#+begin_src conf
    file-history = git show --oneline -s $(git rev-list HEAD -- 32-factures-creation.R)
    file-history-f = "!f() { git show --oneline -s $(git rev-list --date-order HEAD -- $1); }; f"
    file-history-sh = !sh -c 'git show --oneline -s $(git rev-list --date-order HEAD -- $1)'
#+end_src
...
One potential issue is that ~--follow~ is a Horrible Hack that doesn't quite
work. It's good enough for enough purposes that people use it, and it's good
enough that nobody has bothered to fix it, but it misfires sometimes. Making it
correct would take a lot of thinking, and maybe a lot of CPU time in ~git
log~. (WRT the other question's comments, ~git rev-list~ doesn't have a ~--follow~,
so this is one case where you must use ~git log~.)
...
#+begin_src conf
    file-history-follow = log --follow --date-order -C
#+end_src
...
#+begin_src conf
    related = !sh -c 'git log --follow --format=format:%H $1 \
...
#+begin_src conf
    related-files = git show --name-only $(git rev-list HEAD -- 32-factures-creation.R)
    related-files-f = "!f() { git show --name-only $(git rev-list HEAD -- $1); }; f"
    related-files-sh = !sh -c 'git show --name-only $(git rev-list HEAD -- $1)'
#+end_src
...
#+begin_src conf
    file-last-modified = log -n 1 --follow --all --date-order -C
#+end_src
...
#+begin_src conf :tangle no
# Add any directories, files, or patterns you don't want to be tracked by
# version control
#+end_src
...
#+begin_src conf
    ...
    incoming-on-branch = !sh -c 'git fetch' && git log HEAD..$(git origin)/$1
#+end_src

#+begin_src conf
    incoming-files = !(git remote update -p && git log --stat ..@{u})
...
*** Sync branch (merge master into your branch)
...
#+begin_src conf
    export-branch = !git format-patch --stdout $1 > ~/$(git current-branch-without-slash).diff
#+end_src
