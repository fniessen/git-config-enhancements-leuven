...
and right side commit SHAs and substitutes that in for the left-side SHA. The
right-side SHA remains intact.
#+end_note
...
#+begin_src conf
    outgoing-files = log --stat --date=short @{u}..
...
#+begin_src conf
    ...
    outgoing-all = log --branches --not --remotes --decorate --date=short
